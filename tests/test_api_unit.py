...
async def mock_load_crate_metadata(crate_path: UploadFile):
    ...
    return {"result": "success", "crate_url": f"http://minio:9000/crates/{crate_path.filename}"}
...
def mock_list_rde_graphs():
    return {"result": [("http://example.com/rde1", "graph1"), ("http://example.com/rde2", "graph2")]}
...
def mock_backtrack_fn(file_uri: str = None, result_id: str = None):
    ...
    return {"result": [{"action": "action1", "objects": ["object1"], "results": ["result1"]}]}
